...
export const getSourceSetting = async (): Promise<{ mode: number; source: string }> => {
  return await axios.get('https://imgproxy.hieubq.io.vn/comics.php/source').then((res) => res.data)
}
...
export class BaseScraper {
  ...
  protected getComicId(link?: string): string {
    if (!link) return ''
    return link?.match(/\/([^/]+)-\d+$/)?.[0].replace('/', '')
  }
  protected getGenreId(link?: string): string {
    if (!link) return ''
    return link?.match(/[^/]+$/)?.[0]
  }
  ...
  protected async getComics(path: string, page = 1, statusKey: Status = 'all'): Promise<any> {
    const keys: any = {
      'Thể loại': 'genres',
      'Tình trạng': 'status',
    ...
    try {
      ...
      const total_pages =
        $('a[title="Trang cuối"]')?.attr('href')?.split('=').at(-1) || $('.pagination-outter li.active a').text() || 1
  ...
  public async getRecommendComics(type: 'hot' | 'boy' | 'girl' = 'hot'): Promise<any> {
    ...
    const comics = Array.from($('#ctl00_divAlt1 div.item')).map((item) => {
      ...
      const thumbnail = $('img', item).attr('data-src')
  ...
  public async getRecentUpdateComics(page = 1): Promise<any> {
    try {
  ...
  public async getCompletedComics(page = 1): Promise<any> {
    try {
  ...
  public async getNewComics(status: Status = 'all', page = 1): Promise<any> {
    try {
  ...
  public async getComicsByGenre(genreId: string, page = 1, status: Status = 'all'): Promise<any> {
    try {
  ...
  public async getTopDailyComics(status: Status = 'all', page = 1): Promise<any> {
    try {
  ...
  public async getTopWeeklyComics(status: Status = 'all', page = 1): Promise<any> {
    try {
  ...
  public async getTopMonthlyComics(status: Status = 'all', page = 1): Promise<any> {
    try {
  ...
  public async getTopFollowComics(status: Status = 'all', page = 1): Promise<any> {
    try {
  ...
  public async getTopCommentComics(status: Status = 'all', page = 1): Promise<any> {
    try {
  ...
  public async getTopAllComics(status: Status = 'all', page = 1): Promise<any> {
    try {
  ...
  public async getTopChapterComics(status: Status = 'all', page = 1): Promise<any> {
    try {
  ...
  public async getTrendingComics(page = 1): Promise<any> {
    try {
  ...
  public async getBoyComics(page = 1): Promise<any> {
    try {
  ...
  public async getGirlComics(page = 1): Promise<any> {
    try {
  ...
  public async searchComics(query: string, page = 1): Promise<any> {
    try {
      return await this.getComics(`tim-truyen?keyword=${query.replace(/\s+/g, '+')}&`, page)
    } catch (err) {
      throw err
    }
  }
    ...
    try {
      ...
      const images = Array.from($('.page-chapter img')).map((img) => {
        ...
        const host = $(img)
          .attr('src')
          .match(/^\/\/([^/]+)/)[0]
        const src = `/images/get?mode=1&src=https:${$(img).attr('src')}`
  ...
  public async getComments(comicId: string, page = 1, chapterId = -1): Promise<any> {
    try {
      ...
      const comments = Array.from($('.clearfix')).map((item) => {
        ...
        const vote_count = $('.comment-footer .vote-up-count', item).first().text()
        const stickers = Array.from($(item).find('> .summary > .info > .comment-content > img')).map(
          (img) =>
            $(img)
              .attr('src')
              ?.match(/url=(.*)$/)?.[1]
        )
        const created_at = $('.comment-footer abbr', item).first().attr('title')
        const replies = Array.from($('.item', item)).map((reply) => {
          ...
          const content = $('.comment-content', reply).clone().children().remove().end().text().trim()
  ...
  public async getSearchSuggest(query: string): Promise<any> {
    try {
      ...
      const { data } = await axios.get(`${this.domain}/Comic/Services/SuggestSearch.ashx?q=${query}`, {
        headers: { 'User-Agent': this.agent },
      })
      const $ = load(data)
